...
function Dashboard() {
  ...
  const fetchJobs = async () => {
    try {
      const res = await axios.get('http://localhost:5000/api/jobs', { headers: getHeaders() });
      setJobs(res.data);
    } catch (err) {
      console.log(err);
    }
  ...
  const deleteJob = async (id) => {
    try {
      await axios.delete(`http://localhost:5000/api/jobs/${id}`, { headers: getHeaders() });
      fetchJobs();
    } catch (err) {
      console.log(err);
    }
  ...
  const generateCoverLetter = async () => {
    try {
      setAiLoading(true);
      const res = await axios.post('http://localhost:5000/api/ai/cover-letter',
        ...
        {showAI && (
          <div className="bg-white rounded-xl p-6 shadow-sm mb-6 border-l-4 border-purple-500">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              />
              <input
                placeholder="Your Experience (e.g. 1 year Frontend Developer at XYZ)"
                value={aiJob.experience}
            ...
            {coverLetter && (
              <div className="mt-4">
                <div className="bg-purple-50 rounded-lg p-4 text-gray-700 text-sm whitespace-pre-wrap">
                  {coverLetter}
                </div>
                <button
